...
const xemChiTietDon = async (orderId) => {
  try {
    ...
    const { id_order, id_user, products, total, address, date, pay, status, checkedAt, checkedBy, delicercheckedAt, delicercheckedBy } = order;
    ...
    const productHtml = products.map(product => {
      ...
      return `
        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <img src="${imageUrl || 'https://via.placeholder.com/90'}" style="width: 90px; height: 90px; object-fit: cover; border-radius: 6px; margin-right: 10px;">
          <div>
            <strong>${productData.nameproduct || '---'}</strong><br>
            <small>Thể loại: ${category}</small><br>
            <small>Màu: ${color || 'Không có'} - Size: ${product.size || 'Không có'} - SL: ${product.quantity || 0}</small>
          </div>
        </div>
      `;
    }).join('');

    // Xử lý thời gian và thông tin
    const formatDateTime = (dateValue) => {
      if (!dateValue || dateValue === "Chưa cập nhật") return "Chưa cập nhật";
      const date = new Date(dateValue);
      return !isNaN(date) ? date.toLocaleString('vi-VN') : "Chưa cập nhật";
    };
    ...
    const bodyHtml = `
      ...
      <p><strong>Ngày đặt:</strong> ${formatDateTime(date)}</p>
      <p><strong>Thanh toán:</strong> ${pay || '---'}</p>
      <p><strong>Tổng tiền:</strong> ${Number(total || 0).toLocaleString('vi-VN')} ₫</p>
      <p><strong>Trạng thái:</strong> ${status || '---'}</p>
      <p><strong>Thời gian xác nhận:</strong> ${formatDateTime(checkedAt)}</p>
      <p><strong>Người xác nhận:</strong> ${checkedBy || 'Chưa cập nhật'}</p>
      <p><strong>Thời gian giao:</strong> ${formatDateTime(delicercheckedAt)}</p>
      <p><strong>Người giao:</strong> ${delicercheckedBy || 'Chưa cập nhật'}</p>
      <hr>
      <h6>Sản phẩm:</h6>
      ${productHtml || '<p>Không có sản phẩm.</p>'}
    `;
    ...
    document.getElementById('order-detail-body').innerHTML = '<p>Lỗi khi tải chi tiết đơn hàng. Kiểm tra console.</p>';
  }
};
